...
import {Product, Category, Series as model} from "./../models";
...
class SeriesController {
  ...
  public async findOne(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const dbresponse = await model.findByPk(req.params.seriesId, {include: includeModel});
      if (!dbresponse) {
        throw "empty";
      }
      res.json(dbresponse);
  ...
  public async update(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const [updatedRows] = await model.update(req.body, {where: {seriesId: req.params.id}});
      if (updatedRows > 0) {
  ...
  public async destroy(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const deletedRows = await model.destroy({where: {seriesId: req.params.id}});
      if (deletedRows > 0) {
        res.status(204).send();
      } else {
        res.status(404).send("Series not found");
